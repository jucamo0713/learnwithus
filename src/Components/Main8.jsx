...
class Main8 extends React.Component {
    ...
    getInfoCursoCreados = async () => {
        console.log("AH BIEN");
        axios.get(`${this.props.rback}/Cur/get_cursos-Comunidad_Integrado/Curso/${this.props.location.state.idCursoC}`)
            .then(res => {
                this.setState({
    ...
    putInfoRequerida = (form) => {
        axios.put(`${this.props.rback}/Cur/put_cursos_infoBasica/CrearCurso/${this.state.DataInfoCurso[0].id}`, form)
            .then(res => {
            }).catch(err => {
                if (err) {
                    console.error(err);
    ...
    putSetNullContenidoD = () =>{
        axios.put(`${this.props.rback}/Cur/put_cursos_contenido-d-text_setNull/CrearCurso/${this.state.DataInfoCurso[0].id}`)
            .then(res =>{
            }).catch(err =>{
                if(err){
                    console.error(err);
    ...
    deleteContenidoEvaluativo = async () =>{
        axios.delete(`${this.props.rback}/Cur/delete-preguntas-informacion/CrearCurso/${this.state.DataInfoCurso[0].id}`)
            .then(res =>{
    ...
    putCantidadPreguntas = async () =>{
        ...
        axios.put(`${this.props.rback}/Cur/put_cantidad_contenido-e/CrearExamen/${this.state.DataInfoCurso[0].id}&${cero}`)
        .then(res =>{})
        .catch(err =>{
            if(err){
                console.error(err);
    ...
    putPrivacidad = () => {
        ...
        if (titulo.value != "" && tematica.value != "" && materia.value != "") {
            axios.put(`${this.props.rback}/Cur/put_cursos_privacidad/CrearCurso/${this.state.DataInfoCurso[0].id}&${privacidad}`)
                .catch(err => {
                    if (err) {
                        console.error(err);
                    }
    ...
    putSetNullContenidoT = async () => {
        await axios.put(`${this.props.rback}/Cur/put_cursos_contenido-t_setNull/CrearCurso/${this.state.DataInfoCurso[0].id}`)
            .then(res => {
    ...
    deleteCurso = async (numero) => {
        await axios.delete(`${this.props.rback}/Cur/delete-curso-informacion/paginas/${this.state.DataInfoCurso[0].id}&${UsuarioI[0].id_usuario}`)
            .then(res => {
            }).catch(err => {
                if (err) {
                    console.error(err);
