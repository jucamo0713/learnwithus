...
class Main3 extends React.Component {
    ...
    getCursos = async () => {
        return axios.get(`${this.props.rback}/Cur/get_cursos-Comunidad_Integrado/${this.props.location.state.pagina}`)
            .catch(err => {
                console.error(err);
            });
    }
    ...
    postIniciarCurso = async (prop) => {
        await axios.post(`${this.props.rback}/UsuCur/user-init-curso/${prop}`, this.state.form)
            .then(response => {
                console.log("Hola");
                this.setState({
                    IniciarCursoPost: {
    ...
    putCalificacionCurso = async (prop) => {
        /*UsuarioCalificación*/
        await axios.put(`${this.props.rback}/UsuCur/Put_Usuario-calificacion/Comunidad/${UsuarioI[0].id_usuario}&${prop}`)
            .then(res => {
    ...
    putCalificacionCurso2 = async (prop, prop2, prop3) => {
        ...
        document.getElementById("carga").style.display = "block";
        /*Cursos*/
        await axios.put(`${this.props.rback}/Cur/put_cursos_valoracion/comunidad/${prop}`, form2)
            .then(async (res) => {
                let C = await this.getCursos();
                let Ci = await this.getCursoIniciado();
                this.filtrando(C.data, Ci.data);
