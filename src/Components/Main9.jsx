...
class Juego extends React.Component {
    ...
    getcalificacion = async () => {
        await axios.get(`${this.props.rback}/UsuCur/traer-calificacion/Examen/${UsuarioI[0].id_usuario}&${this.props.location.state.id}`)
            .then(res => {
    ...
    putEvaluacion = async (prop) => {
        await axios.put(`${this.props.rback}/UsuCur/Put_Usuario-calificacion_calificacion/Comunidad/${UsuarioI[0].id_usuario}&${this.props.location.state.id}&${prop}`)
            .then(res => {
            }).catch(err => {
                console.log(err);
            });
