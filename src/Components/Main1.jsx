...
class Main1 extends React.Component {
    ...
    getUserNameRegistro = async (UserName) => {
        const response = await axios.get(`${this.props.rback}/Usu/usuario-sesion/${UserName.value}`)
    ...
    getCorreoRegistro = async (Correo) => {
        const response = await axios.get(`${this.props.rback}/Usu/correo-sesion/${Correo.value}`)
        if (response.length > 0) {
    ...
    getUserNameInicioSesion = async (Usuario, Contraseña) => {
        await axios.get(`${this.props.rback}/Usu/usuario-sesion/${Usuario.value.toLowerCase()}`)
            .then(res => {
    ...
    postUser = async (form) => {
        await axios.post(`${this.props.rback}/Usu/registro-sesion`, form)
            .then(res => {
    ...
    putEdad = (edad) => {
        ...
        axios.put(`${this.props.rback}/Usu/put-usuarios-edad/${this.state.data[0].id_usuario}`, form);
    }
    render() {
        return (
            <>
