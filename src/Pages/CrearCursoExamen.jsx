...
class CrearCursoTeorico extends React.Component {
    ...
    getContenidoE = async () => {
        await axios.get(`${this.props.rback}/Cur/get_preguntas_informacion/ContenidoE/${this.props.location.state.idCursoC}`)
            .then(res => {
    ...
    putContenidoE = async () => {
        if (this.state.cantp == 0) {
            ...
            document.getElementById("carga").style.display = "none";
            await axios.delete(`${this.props.rback}/Cur/delete-preguntas-informacion/CrearCurso/${this.props.location.state.idCursoC}`)
                .then(async (res) => {
                    for (let x = 0; this.state.ExamenCurso.length > x; x++) {
                        ...
                        axios.post(`${this.props.rback}/Cur/post_preguntas_curso/CrearExamen`, form)
                            .then(res => {
                                console.log(res.data);
                            }).catch(err => {
                                if (err) {
            ...
            axios.put(`${this.props.rback}/Cur/put_cantidad_contenido-e/CrearExamen/${this.props.location.state.idCursoC}&${document.getElementById("cantidad").value}`)
                .then(res => {
                    console.log(res.data);
                }).catch(err => {
                    if (err) {
    ...
    cantidadPreguntas = async () => {
        return axios.get(`${this.props.rback}/Cur/get_cursos_cantidadPreguntas/CrearContenidoE/${this.props.location.state.idCursoC}`)
            .catch(err => {
                if (err) {
                    console.error(err);
                }
