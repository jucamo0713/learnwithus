...
class Didactico extends React.Component {
    ...
    async componentDidMount() {
        await axios.get(`${this.props.rback}/Fil/get-Examne-Contenido/Didactico/${this.props.location.state.id}&${UsuarioI[0].usuario}`)
            .then(res => {
    ...
    componentWillUnmount() {
        axios.get(`${this.props.rback}/Fil/get-Examne-Contenido-delete/Didactico/${this.props.location.state.id}&${UsuarioI[0].usuario}`)
            .then(res => {
    ...
    iframe = () => {
        if (this.state.DataCurso[0]?.contenido_d_text != null && this.state.DataCurso[0]?.contenido_d_text != "") {
            return (
                <iframe className="formGames" src={`${this.props.rback}/Fil/file-Didactico/${UsuarioI[0].usuario}/`}></iframe>
            );
        } else {
            return (
